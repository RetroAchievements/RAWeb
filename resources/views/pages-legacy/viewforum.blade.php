...
<x-app-layout pageTitle="Forum: {{ $thisForumTitle }}">
    ...
    foreach ($topicList as $topicData) {
        ...
        echo "<td>";
        ?>
        <a href="{{ route('forum.topic', ['forumTopic' => $nextTopicID]) }}">{{ $nextTopicTitle }}</a>
        <?php
        ...
        <a
            class="text-2xs"
            href="{{ route('forum.topic', ['forumTopic' => $nextTopicID, 'comment' => $nextTopicLastCommentID]) }}#{{ $nextTopicLastCommentID }}"
            title="View latest post"
        >
            View
        </a>
        <?php
