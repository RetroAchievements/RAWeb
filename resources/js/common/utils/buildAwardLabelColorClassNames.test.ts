...
describe('Util: buildAwardLabelColorClassNames', () => {
  ...
  it('given the consumer is using the "base" variant, returns all correct classes for the various awards', () => {
    ...
    const gameBeatenCasual = buildAwardLabelColorClassNames('game_beaten', 0, 'base');
    ...
    expect(gameBeatenCasual).toEqual('text-zinc-400');
  });
  ...
  it('given the consumer is using the "muted-group" variant, returns all correct classes for the various awards', () => {
    ...
    const gameBeatenCasual = buildAwardLabelColorClassNames('game_beaten', 0, 'muted-group');
    ...
    expect(gameBeatenCasual).toEqual('transition text-muted group-hover:text-zinc-400');
  });
});
