...
import { UserModerationCommentsMainRoot } from './UserModerationCommentsMainRoot';

describe('Component: UserModerationCommentsMainRoot', () => {
  beforeEach(() => {
  ...
  it('displays an accessible heading', () => {
    ...
    // ASSERT
    expect(screen.getByRole('heading', { name: /moderation comments/i })).toBeVisible();
  });
  ...
  it('given the user adds a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['user.moderation-comment.index', { user: 'Scott', _query: { page: 3 } }],
      { preserveScroll: true },
    );
  });
  ...
  it('given the user deletes a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['user.moderation-comment.index', { user: 'Scott', _query: { page: 1 } }],
      { preserveScroll: true },
    );
  });
});
