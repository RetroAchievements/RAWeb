...
import { AchievementCommentsMainRoot } from './AchievementCommentsMainRoot';

describe('Component: AchievementCommentsMainRoot', () => {
  beforeEach(() => {
  ...
  it('displays achievement breadcrumbs', () => {
    // ARRANGE
    ...
    expect(screen.getByRole('listitem', { name: achievement.title })).toBeVisible();
  });
  ...
  it('given the user paginates, changes the current route correctly', async () => {
    ...
    render<App.Community.Data.AchievementCommentsPageProps>(<AchievementCommentsMainRoot />, {
      pageProps: {
        paginatedComments,
        auth: null,
        achievement: createAchievement({ id: 1 }),
        isSubscribed: false,
        canComment: true,
      },
    });
  ...
  it('given the user adds a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['achievement.comment.index', { achievement: 1, _query: { page: 3 } }],
      { preserveScroll: true },
    );
  });
  ...
  it('given the user deletes a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['achievement.comment.index', { achievement: 1, _query: { page: 1 } }],
      { preserveScroll: true },
    );
  });
});
