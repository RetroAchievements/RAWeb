...
import { GameModificationCommentsMainRoot } from './GameModificationCommentsMainRoot';

describe('Component: GameModificationCommentsMainRoot', () => {
  beforeEach(() => {
  ...
  it('given the user adds a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['game.modification-comment.index', { game: 1, _query: { page: 3 } }],
      { preserveScroll: true },
    );
  });
  ...
  it('given the user deletes a comment, changes the current route correctly', async () => {
    ...
    expect(visitSpy).toHaveBeenCalledOnce();
    expect(visitSpy).toHaveBeenCalledWith(
      ['game.modification-comment.index', { game: 1, _query: { page: 1 } }],
      { preserveScroll: true },
    );
  });
});
