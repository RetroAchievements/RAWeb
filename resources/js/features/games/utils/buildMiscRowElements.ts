...
export function buildMiscRowElements(
  ...
  const uncategorizedHubs = nonEventHubs.filter((hub) => {
    ...
    // Exclude hubs that appear in the sidebar (Series, Meta, Rollout Sets, Achievement Extras) from misc categorization.
    if (
      title.includes('series -') ||
      title.includes('meta|') ||
      title.includes('meta -') ||
      title.includes('rollout sets -') ||
      title.includes('ranews -') ||
      title.includes('custom awards -')
    ) {
      return false;
    }
