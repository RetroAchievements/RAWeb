...
describe('Util: buildMiscRowElements', () => {
  ...
  it('excludes Meta descriptive hubs', () => {
    // ARRANGE
    const metaDescriptiveHub = createGameSet({
      id: 123,
    ...
    const allGameHubs = [metaDescriptiveHub, customHub];
    const usedHubIds = new Set<number>();
  ...
  it('excludes Rollout Sets hubs', () => {
    // ARRANGE
    const rolloutSetsHub = createGameSet({
      id: 123,
      title: '[Rollout Sets - Wave 1]',
      type: 'hub',
    });
    const customHub = createGameSet({
      id: 456,
    ...
    const allGameHubs = [rolloutSetsHub, customHub];
    const usedHubIds = new Set<number>();
  ...
  it('excludes Achievement Extras hubs', () => {
    // ARRANGE
    const raNewsHub = createGameSet({
      id: 123,
      title: '[RANews - 2023-01]',
      type: 'hub',
    });
    const customAwardsHub = createGameSet({
      id: 456,
      title: '[Custom Awards - Best Graphics]',
      type: 'hub',
    });
    const customHub = createGameSet({
      id: 789,
    ...
    const allGameHubs = [raNewsHub, customAwardsHub, customHub];
    const usedHubIds = new Set<number>();
    ...
    // ASSERT
    expect(result).toEqual([{ label: 'Something', hubId: 789, href: ['hub.show', 789] }]);
  });
