...
/*$patreonSignUnhashed = hash_hmac( 'md5', $patreonSignature, PATREON_CLIENT_SECRET, false );
error_log( $patreonSignUnhashed );

$patreonSignUnhashed = hash_hmac( 'md5', $patreonSignature, PATREON_CLIENT_SECRET, true );
error_log( $patreonSignUnhashed );
function verify_webhook( $patreonSignature, $hmac_header )
{
    error_log( hash_hmac( 'sha256', $patreonSignature, PATREON_CLIENT_SECRET, true ) );
    error_log( json_decode( hash_hmac( 'sha256', $patreonSignature, PATREON_CLIENT_SECRET, true ) ) );
    $calculated_hmac = base64_encode( hash_hmac( 'sha256', $patreonSignature, PATREON_CLIENT_SECRET, true ) );
