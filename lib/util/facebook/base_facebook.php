...
abstract class BaseFacebook
{
    ...
    protected function getApiUrl($method)
    {
      ...
      ['admin.getallocation' => 1,
        ...
        if (isset($READ_ONLY_CALLS[mb_strtolower($method)])) {
            $name = 'api_read';
        } elseif (mb_strtolower($method) == 'video.upload') {
            $name = 'api_video';
        }
    ...
    protected function shouldRetainParam($param)
    {
        foreach (self::$DROP_QUERY_PARAMS as $drop_query_param) {
            if (mb_strpos($param, $drop_query_param . '=') === 0) {
                return false;
            }
        }
    ...
    protected function throwAPIException($result)
    {
      ...
      case 'Exception':
        ...
        if ((mb_strpos($message, 'Error validating access token') !== false) ||
            (mb_strpos($message, 'Invalid OAuth access token') !== false) ||
            (mb_strpos($message, 'An active access token must be used') !== false)
        ) {
            $this->destroySession();
        }
        break;
