...
function UploadNewAchievement( $author, $gameID, $title, $desc, $progress, $progressMax, $progressFmt, $points, $mem, $type, &$idInOut, $badge, &$errorOut )
{
    ...
    if( !isset( $idInOut ) || $idInOut == 0 )
    {
        ...
        if( s_mysql_query( $query ) !== false )
        {
            ...
            addArticleComment( $author, 2, $idInOut, "\"$author\" uploaded this achievement." );
    ...
    else
    {
        ...
        if( $dbResult !== false && mysqli_num_rows( $dbResult ) == 1 )
        {
            ...
            if( s_mysql_query( $query ) !== false )
            {
                ...
                {
                    if( $type == 3 ) 
                        addArticleComment( $author, 2, $idInOut, "\"$author\" promoted this achievement to the Core set." );
                    else if( $type == 5 ) 
                        addArticleComment( $author, 2, $idInOut, "\"$author\" demoted this achievement to Unofficial." );
                }
                else
                    addArticleComment( $author, 2, $idInOut, "\"$author\" edited this achievement." );
