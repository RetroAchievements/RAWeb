...
function RenderAwardGroup(array $awards, string $title, string $awardsOwnerUsername): void
{
    ...
    echo "<div class='component w-full bg-embed gap-2 grid grid-cols-auto-fill-award-grid max-sm:grid-cols-5 max-sm:max-w-xs max-sm:px-4 max-sm:py-2 lg:gap-x-0 xl:py-2 xl:px-4'>";
    $imageSize = 48;
    foreach ($awards as $award) {
        if ($award['DisplayOrder'] >= 0) {
