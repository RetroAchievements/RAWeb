        ...
        if ($info == 0) {
            if ($unlockMode == UnlockMode::Hardcore) {
                $selectQuery = "SELECT ua.User,
                        (SELECT COALESCE(SUM(CASE WHEN aw.HardcoreMode = " . UnlockMode::Hardcore . " AND ach.Flags = " . AchievementType::OfficialCore . " THEN 1 ELSE 0 END), 0) FROM Awarded AS aw JOIN Achievements AS ach ON aw.AchievementID = ach.ID JOIN GameData as gd ON ach.GameID = gd.ID WHERE aw.User = ua.User AND gd.ConsoleID NOT IN (100, 101)) AS AchievementCount,
                ...
                $selectQuery = "SELECT ua.User,
                        (SELECT COALESCE(SUM(CASE WHEN aw.HardcoreMode = " . UnlockMode::Softcore . " AND ach.Flags = " . AchievementType::OfficialCore . " THEN 1 ELSE -1 END), 0) FROM Awarded AS aw JOIN Achievements AS ach ON aw.AchievementID = ach.ID JOIN GameData as gd ON ach.GameID = gd.ID WHERE aw.User = ua.User AND gd.ConsoleID NOT IN (100, 101)) AS AchievementCount,
