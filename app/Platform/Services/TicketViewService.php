...
class TicketViewService
{
    ...
    public function load(Ticket $ticket): void
    {
        if ($ticket->reporter) {
            ...
            $msgPayload = "Hi [user={$ticket->reporter->username}], I'm contacting you about [ticket={$ticket->ID}]";
            $msgPayload = rawurlencode($msgPayload);
            $this->contactReporterUrl = route('message-thread.create') . "?to={$ticket->reporter->username}&subject=$msgTitle&message=$msgPayload";
