 ...
 * @property GameInviteStatus $status
 ...
 */
class GameInvite extends BaseModel
{
    use HasFactory;
    ...
    protected static function boot()
    {
        ...
        static::creating(function (self $invite) {
            $invite->sent_at ??= now();
            $invite->status ??= GameInviteStatus::Pending;
        ...
        static::updating(function (self $invite) {
            ...
            if ($invite->isDirty('status') &&
                $invite->getOriginal('status') === GameInviteStatus::Pending->value &&
                $invite->status !== GameInviteStatus::Pending) {
     ...
     */
    public function scopeWithStatus(Builder $query, GameInviteStatus $status): Builder
    {
        return $query->where('status', $status->value);
    }

    /**
     * Scope to invites for a specific game.
     */
    ...
    /**
     * Scope a query to filter invites by role (sent/received).
     *
     * @param Builder $query
     * @param string $role
     * @param User $user
     * @return Builder
     */
    public function scopeForRole($query, string $role, User $user)
    {
        if ($role === 'sent') {
            return $query->where('sender_user_id', $user->id);
        }
    ...
    public function canBeActedOnBy(User $user): bool
    {
        ...
        if ($this->sender_user_id === $user->id && $this->status === GameInviteStatus::Pending) {
            return true;
        }

        // Recipient can accept/decline pending invites
        if ($this->recipient_user_id === $user->id && $this->status === GameInviteStatus::Pending) {
            return true;
        }
