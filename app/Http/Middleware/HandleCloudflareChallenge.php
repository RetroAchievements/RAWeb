...
class HandleCloudflareChallenge
{
    ...
    {
        // Quickly look for Cloudflare indicators in cookies/headers/URL first.
        // This avoids processing POST data for 99.9% of normal requests.
        $hasCfClearance = $request->hasCookie('cf_clearance');
        $hasCfChallengeToken =
            str_contains($request->header('referer', ''), '__cf_chl_tk=')
            || str_contains($request->fullUrl(), '__cf_chl_tk=');
