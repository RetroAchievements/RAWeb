...
$reqAchievementID = 1;
$reqAchievementValidation = sprintf( "%d,%d-%s.%s-%d132%s2A%slLIA", $reqAchievementID, (strlen( $testUser ) * 3) + 1, $testUser, $appToken, $reqAchievementID, $testUser, "WOAHi2" );
...
else if( seekGET( 'action' ) == 'reconstructsiteawards' )
{
...
else if( seekGET( 'action' ) == 'recalcsiteawards' )
{
    $tgtPlayer = seekGET( 't', NULL );
    {
    ...
    }
    {
        ...
        $userList = '';
        while( $db_entry = mysqli_fetch_assoc( $dbResult ) )
            $userList[] = array( $db_entry[ 'User' ], $db_entry[ 'ContribCount' ], $db_entry[ 'ContribYield' ], $db_entry[ 'fbUser' ] );
...
else if( seekPOST( 'action' ) == 'getachids' )
{
...
else if( seekPOST( 'action' ) == 'giveaward' )
{
    ...
    if( isset( $awardAchievementID ) && isset( $awardAchievementUser ) )
    {
        ...
        {
            if( addEarnedAchievement( $awardAchievementUser, '', $nextID, 0, $newPointTotal, $awardAchHardcore, FALSE ) )
            {
...
else if( seekGET( 'action' ) == 'recalctrueratio' )
{
    set_time_limit( 3000 );
...
else if( seekGET( 'action' ) == 'recalcplayerscores' )
{
    set_time_limit( 3000 );
...
else if( seekPOSTorGET( 'action' ) == 'updatestaticdata' )
{
...
// exit;
// }
else if( seekPOSTorGET( 'action' ) == 'errorlog' )
{
