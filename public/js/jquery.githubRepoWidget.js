;jQuery(document).ready(function($){
	...
	$('.github-widget').each(function(){
		...
		$.ajax({
			url: 'https://api.github.com/repos/RetroAchievements/RASuite/commits',
			//url: 'https://api.github.com/repos/' + repo,
			dataType: 'jsonp',
			success: function(results) {
				var repo = results.data[0], date, pushed_at = 'unknown';
				
				var commit = repo.commit;
				var msg = commit.message;
				var at = repo.commit.committer.date;
				...
				$widget.find('.description span').text(repo.commit.message);
				//$widget.find('.updated').html('Latest commit to the <strong>' + repo.default_branch + '</strong> branch on ' + pushed_at);
				$widget.find('.updated').html('Latest commit was at ' + pushed_at);
