#<?php 
	...
	if( RA_ReadCookieCredentials( $user, $points, $truePoints, $unreadMessageCount, $permissions, \RA\Permissions::SuperUser ) )
	{
		if( isset( $richPresence ) )
		{
			...
			header( "location: " . APP_URL . "/Game/$gameID?e=ok" );
			exit;
		}
        ...
        else if( isset( $newForumTopic ) )
        {
            ...
            {
			    header( "location: " . APP_URL . "/Game/$gameID?e=error" );
                exit;
            }
        }
		else
		{
			//	unknown?
			header( "location: " . APP_URL . "/Game/$gameID?e=unrecognised" );
			exit;
		}
	}
	else
	{
		header( "location: " . APP_URL . "/Game/$gameID?e=notloggedin" );
		exit;
	}
?>
